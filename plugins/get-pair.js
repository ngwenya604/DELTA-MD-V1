...
async (conn, mek, m, { from, prefix, quoted, q, reply }) => {
    try {
        ...
        // Validate input
        if (!q) {
            return await reply("*Example -* .pair +23727582XX");
        }

        // Fetch pairing code
        //const fetch = require("node-fetch");
        const response = await fetch(`https://prince-mdx-session2.onrender.com/pair?phone=${q}`);
        ...
        const doneMessage = "> DELTA MDX PAIR COMPLETED*";
